...
void http_listener::do_accept() {
  LOG("[http_listener@do_accept] scope_in");
  acceptor_.async_accept(
      make_strand(ioc_),
      [self = shared_from_this()](const boost::system::error_code& error,
                                  boost::asio::ip::tcp::socket socket) {
        if (!error) {
          LOG("[http_listener@do_accept] accepted");
          ...
          boost::make_shared<http_session>(self->core_, self->server_id_,
                                           _connection)
              ->start();
        }

        LOG("[http_listener@do_accept] continue");
        self->do_accept();
      });
  LOG("[http_listener@do_accept] scope_out");
}
}  // namespace copper::components
