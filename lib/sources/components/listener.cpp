...
namespace copper::components {
    ...
    > listener(
      ...
      auto acceptor = typename boost::asio::ip::tcp::acceptor::rebind_executor<boost::asio::strand<boost::asio::io_context::executor_type>>::other{
        executor, endpoint};
      ...
      while (!cs.cancelled()) {
        ...
        auto session = state
          ->get_database()
          ->create_session(socket.remote_endpoint().address().to_string(), socket.remote_endpoint().port());
        ...
        boost::asio::co_spawn(
          ...
          task_group->adapt(
            [session, &state](std::exception_ptr e) {
              if (e) {
                // LCOV_EXCL_START
                try {
