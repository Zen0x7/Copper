  ...
  acceptor_.async_accept(
      ...
      [self = shared_from_this()](const boost::system::error_code& error,
        ...
        if (!error) {
          ...
          auto& stream = std::get<connection::tcp_stream>(_connection->stream_);
          _connection->upgrade_to_websocket(stream.get_executor());
#ifdef DEBUG_ENABLED
          self->core_->database_->create_connection(self->core_, _connection);
          self->core_->database_->connection_is_websocket(self->core_,
                                                          _connection->id_);
#endif
          self->core_->state_->connected(_connection);
          boost::make_shared<websocket_session>(self->core_, self->server_id_,
                                                _connection)
              ->start();
        }
