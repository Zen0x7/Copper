...
TEST(Components_Base64, Encode) {
  ...
  const std::string _output = base64_encode(_input);
  ASSERT_EQ(_output, "aGVsbG8rd29ybGQvLw==");

  const std::string _output_no_padding = base64_encode(_input, false);
...
TEST(Components_Base64, Decode) {
  ...
  const std::string _output = base64_decode(_input);
  ASSERT_EQ(_output, "hello+world//");

  const std::string _input_no_padding = "aGVsbG8rd29ybGQvLw";
  const std::string _output_no_padding = base64_decode(_input_no_padding);
