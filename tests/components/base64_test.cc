...
TEST(Components_Base64, Encode) {
    ...
    const std::string output = base64_encode(input);
    ASSERT_EQ(output, "aGVsbG8rd29ybGQvLw==");

    const std::string output_no_padding = base64_encode(input, false);
...
TEST(Components_Base64, Decode) {
    ...
    const std::string output = base64_decode(input);
    ASSERT_EQ(output, "hello+world//");

    const std::string input_no_padding = "aGVsbG8rd29ybGQvLw";
    const std::string output_no_padding = base64_decode(input_no_padding);
