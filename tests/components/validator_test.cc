...
TEST(Components_Validator, OnErrors) {
    ...
    const std::string valid_object = R"({"uuid":"invalid-uuid","uuid_v4":10,"string":7,"string_confirmation":"hello world","strong":"hello","strong_confirmation":"world","streng":"test","strung":"abc","strung_confirmation":10,"number":"10","array_of_strings":[10,[],"hello"],"wrong_array_of_strings":"abc","empty_array_of_strings":[],"object":[]})";
    const auto value = boost::json::parse(valid_object);
    ...
    const std::string invalid_object = R"([10,5,30])";
    const auto invalid_object_value = boost::json::parse(invalid_object);

    const auto invalid_object_response = validator::make(rules, invalid_object_value);
